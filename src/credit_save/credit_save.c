...
int main(int argc, char **argv) {
  ...
  char *host,*p,*p1,passwd[50],passwd1[50],name[50],con_char[10];
  ...
  if (exist_cred(host) == 0) {
    printf(" %s is exists. Do you wish to update or not? (y/n): ",host);
    while (loop--) {
      scanf("%s",con_char);
      getchar();
  ...
  do {
    int ret;
    printf("Enter iLO user name for %s: ", host);
    scanf("%s",name);
    getchar();
  
    set_disp_mode(STDIN_FILENO,0);
  
    printf("Enter iLO user password: ");
    ...
    *p='\0';
    ...
    printf("\n"); 
    printf("Enter iLO user password again:");
    ...
    *p1='\0';
    ...
    if (strlen(passwd) == 0)
      ...
      printf("\n"); 
      set_cred(host,name,passwd);
      //printf("%s",get_cred(host));
      return 0;
    } else
